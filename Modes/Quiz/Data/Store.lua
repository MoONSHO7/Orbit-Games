...
local PREVIOUS_SCHEMA_VERSION = 6
local OLDER_SCHEMA_VERSION = 5
local PREVIOUS_SCORING_SCHEMA_VERSION = 4
local PREVIOUS_SCORING_VERSION = 1
local ARCHIVE_SCORING_VERSION = 2
local ARCHIVE_ANSWER_SECONDS = 15
...
local function CopyAnswer(answer, duration, legacy, choiceCount, scoringVersion)
    ...
    if legacy then
        ...
        local calculate = scoringVersion == PREVIOUS_SCORING_VERSION and PreviousPoints or Quiz.Scoring.CalculateLegacy
...
local function CopyRound(result, legacy, previousScoring)
    ...
    if not legacy then
        ...
        if
            scoringVersion ~= PREVIOUS_SCORING_VERSION and scoringVersion ~= ARCHIVE_SCORING_VERSION
            or previousScoring and scoringVersion ~= PREVIOUS_SCORING_VERSION
            or scoringVersion == ARCHIVE_SCORING_VERSION and result.duration ~= ARCHIVE_ANSWER_SECONDS
        then
            return nil
        end
    end
...
function Store:Normalize(saved)
    ...
    elseif
        ...
        and saved.schemaVersion ~= OLDER_SCHEMA_VERSION
...
function Store:Bind(db)
    self.db = db
    Quiz.PersonalScores:Bind(db.personalScores)
end

function Store:Initialize(saved)
    local db, reason = self:Normalize(saved)
    if not db then
        return nil, reason
    end
    self:Bind(db)
