...
function Model.New(settings, questions, randomFunction, packRules)
...
function Model:PrepareQuestion(uniqueQuestionId)
    if self.state ~= "ready" and self.state ~= "results" then
        return nil, "not_ready"
    end
    if not IsInteger(uniqueQuestionId, 1, MAX_SAFE_INTEGER - 1) or uniqueQuestionId <= self.lastQuestionId then
...
function Model:OpenQuestion(now)
...
function Model:Submit(guid, name, questionId, choiceIndex, now, actionId)
...
function Model:CloseQuestion(now, multiplayer)
    if self.state ~= "open" then
        return nil, "not_open"
    end
    ...
    for guid, answer in pairs(round.answers) do
        ...
        player.score = Quiz.Scoring.Clamp(Quiz.Scoring.Add(player.score, answer.points))
