...
local L = Quiz.L
local Standings = Quiz.Standings
local MAX_PEERS = 16
...
local QUESTION_HEADER_FIELDS = 16
local LETTER_OFFSET = 64
local EXPIRY_GRACE = 2
local PREPARE_TIMEOUT = 20
...
local function TotalScore(text)
    local value = Score(text)
...
local function Elapsed(text, duration)
    if not duration or type(text) ~= "string" or #text > MAX_ELAPSED_BYTES then
        return nil
    end
    ...
    local value = tonumber(text)
    if value and value >= 0 and value <= duration then
        return value
    end
end
...
local function RemoveResult(self, recipient, id)
    Games.Comms:Cancel(ResultTag(recipient.name, id))
...
local function SendRetained(recipient, entry, now)
    ...
    if now < entry.nextRetryAt or Games.Comms:IsTagBusy(tag) then
        return false
    end
    local sent = Send(recipient.name, entry.fields, tag)
    if sent then
...
local function SameMetadata(question, receipt)
    return question.packId == receipt.packId
        ...
        and question.rulesKey == receipt.rulesKey
...
local function RenewMembership(client, view, now)
    ...
    client.answerAttempts, client.retryAt, client.lockSyncId = nil, nil, nil
    ...
    Standings:RenewMembership(Session)
end
...
function Session:Initialize()
    ...
    self.view = { role = "idle", state = "idle", gameTypeId = Quiz.id }
    Standings:Reset(self)
end
...
function Session:GetView()
    if self.hostSession then
        return Quiz.Controller:GetHostView()
    end
...
function Session:GetStandings()
    if self.hostSession then
        return Standings:GetHost(self)
    end
    if self.client then
        return Standings:GetClient(self)
    end
    return nil, 0
end

function Session:SetStandingsVisible(visible)
    local changed = Standings:SetVisible(self, visible)
...
function Session:StartHost(sessionId)
    ...
    Games.Comms:Clear()
    if client then
        Send(client.name, Fields("L", client.session or "", client.request))
    end
    self.client = nil
    ...
    Standings:Reset(self)
    Quiz.StreakSync:Initialize(self)
    Quiz.StreakSync:Register(self, Games.Identity.name)
    Games.Discovery:Advertise()
end

function Session:EndHost(cancelToasts)
    if cancelToasts then
        Quiz.StreakToasts:Clear()
    end
    Games.Discovery:StopHost()
    self:CancelRoundMessages()
    Standings:Cancel(self)
    local now = GetTime()
    for _, peer in pairs(self.peers) do
        ...
        Send(peer.name, Fields("X", self.hostSession))
    end
    self.hostSession = nil
    self.peers = {}
    self.resultRecipients, self.resultRecipientCount, self.resultCount = {}, 0, 0
    self.view = { role = "idle", state = "stopped", gameTypeId = Quiz.id }
    Standings:Reset(self)
end
...
function Session:JoinHost(name, advertisedSession)
    ...
    if self.hostSession then
        Quiz.Controller:Stop()
    else
        self:Leave()
    end
    Quiz.Controller.notice = nil
    local now = GetTime()
    ...
    self.view = { role = "participant", state = "joining", gameTypeId = Quiz.id, hostName = host }
    local sent, sendError = Send(host, Fields("J", request))
    if not sent then
        self.client = nil
...
function Session:SubmitAnswer(input)
    ...
    if not view.rules.allowAnswerChanges and (view.selected or view.confirmedSelected or view.locked) then
        return false, "answer_locked"
    end
    if self.hostSession then
        return Quiz.Controller:AcceptAnswer(Games.Identity.name, Games.Identity.guid, view.id, index)
    end
    local client = self.client
    ...
    view.selected = index
    Games.Comms:Cancel("answer")
    local sent, reason = Send(client.name, AnswerFields(client, view), "answer")
    if not sent then
    ...
    view.locked, view.pending, view.notice = not view.rules.allowAnswerChanges, true, nil
    client.answerAttempts, client.retryAt = 1, GetTime() + ANSWER_RETRY
    return true
end

function Session:CancelRoundMessages()
    Games.Comms:Cancel("streak-name")
    for _, peer in pairs(self.peers) do
        Games.Comms:Cancel(PeerTag(peer.name))
        Games.Comms:Cancel(AnswerTag(peer.name))
    end
end

function Session:SendQuestion(peer)
    local game, round = Quiz.Controller.game, Quiz.Controller.game.round
    return Send(
        peer.name,
        Fields(
            "Q",
...
function Session:SendOpen(peer)
    ...
    if now < round.deadline and (peer.lastOpenId ~= round.id or now - peer.lastOpenAt >= SYNC_INTERVAL) then
        ...
        Send(peer.name, Fields("O", self.hostSession, round.id, self.serverDeadline), PeerTag(peer.name))
        self:SendLock(peer)
    end
end

function Session:SendResult(peer)
    local game, result = Quiz.Controller.game, Quiz.Controller.game.lastResult
    local now = GetTime()
    if
    ...
    if not entry then
        ...
        local fields = Fields(
            ...
            ScoreText(Quiz.Controller:GetScore(peer.playerKey)),
            result.correctCount,
            result.totalAnswers,
...
function Session:RetryResults(now)
    for _, recipient in pairs(self.resultRecipients) do
        ...
        if nextEntry and self.peers[recipient.key] and not Games.Comms:IsBusy(recipient.name) then
...
function Session:Snapshot(peer, now)
    ...
    peer.lastSync = now
    if Games.Comms:IsBusy(peer.name) then
        return
    end
    ...
    if game.state == "paused" then
        Send(
            peer.name,
            Fields("P", self.hostSession, game.lastQuestionId, Quiz.Controller.autoPaused and "restricted" or "manual")
        )
    elseif game.round then
        if peer.readyId ~= game.round.id then
            self:SendQuestion(peer)
        end
        if game.state == "results" or game.state == "finished" then
...
function Session:CanOpen(now)
    ...
    for _, peer in pairs(self.peers) do
        if peer.readyId ~= Quiz.Controller.game.round.id then
            return false
        end
    end
    return true
...
function Session:OpenRound()
    self.serverDeadline = GetServerTime() + Quiz.Controller.game.rules.answerSeconds
    for _, peer in pairs(self.peers) do
        if peer.readyId == Quiz.Controller.game.round.id then
            self:SendOpen(peer)
        end
...
function Session:RevealRound()
    ...
    for playerKey, answer in pairs(round.answers) do
        ...
        if not self.peers[key] and key ~= Games.Identity.name:lower() then
...
function Session:Pause(reason)
    ...
    for _, peer in pairs(self.peers) do
        peer.readyId = nil
        Send(peer.name, Fields("P", self.hostSession, Quiz.Controller.game.lastQuestionId, reason))
    end
end
    ...
    if not restricted then
        ...
        if self.client then
            ...
            if client.session then
                ...
                Send(client.name, Fields("J", client.request))
            else
...
function Session:OnSendError(target, reason)
    if reason == "addon_lockdown" then
        Games.Main.restrictedUntil = GetTime() + SYNC_INTERVAL
    ...
    if self.hostSession then
        if reason == "target_offline" then
            if self.peers[key] then
                Standings:CancelPeer(self.peers[key])
            end
            self.peers[key] = nil
        end
...
function Session:ReceiveHost(sender, fields, now)
    ...
    if code == "J" then
        if not Token(fields[2]) or key == Games.Identity.name:lower() then
            return
        end
        ...
        if not peer then
            ...
            peer = { name = sender, playerKey = Quiz.Controller:PlayerKey(sender), lastSeen = now }
        ...
        if not peer.lastWelcome or now - peer.lastWelcome >= SYNC_INTERVAL then
            if peer.request ~= fields[2] then
                ...
                Standings:CancelPeer(peer)
                Games.Comms:Cancel(PeerTag(sender))
                Games.Comms:Cancel(AnswerTag(sender))
            ...
            Send(sender, Fields("W", fields[2], self.hostSession, ScoreText(Quiz.Controller:GetScore(peer.playerKey))))
        end
        return
    end
    if code == "L" then
    ...
    if
        ...
        or code == "G" and fields[3] ~= peer.request
    ...
    elseif code == "D" then
        ...
        if game.round and id == game.round.id then
            ...
            if game.state == "open" then
                self:SendOpen(peer)
            elseif game.state == "results" or game.state == "finished" then
                self:SendResult(peer)
            end
        end
    elseif code == "A" then
        ...
        local accepted, reason =
            Quiz.Controller:AcceptAnswer(peer.name, nil, id, choice, peer.request .. "." .. fields[5])
        if accepted then
        ...
        elseif ANSWER_ERRORS[reason] and (not peer.lastErrorAt or now - peer.lastErrorAt >= SYNC_INTERVAL) then
            ...
            if reason == "answer_locked" then
                peer.lastLockId = nil
                self:SendLock(peer)
            end
            Send(sender, Fields("E", self.hostSession, id, reason, revision, peer.request), PeerTag(sender))
        end
    end
end
...
function Session:ReceiveQuestion(fields, now)
    ...
    local rules = Quiz.Rules.Decode(fields[16])
    local duration = rules and Integer(fields[7], rules.answerSeconds, rules.answerSeconds)
    ...
    if
        ...
        or self.client.rulesKey and self.client.rulesKey ~= fields[16]
    then
        return false
    end
    local choices, seen = {}, {}
    ...
    if view.id == id then
        if
            ...
            or view.rulesKey ~= fields[16]
            or view.duration ~= duration
            or view.cycle ~= cycle
            or view.number ~= number
            or view.total ~= total
            or view.difficulty ~= (fields[10] ~= "" and fields[10] or nil)
            or view.era ~= (fields[11] ~= "" and fields[11] or nil)
            or #view.choices ~= choiceCount
        then
            return false
        end
        for index, choice in ipairs(choices) do
            if view.choices[index] ~= choice then
                return false
            end
        end
    end
    self.client.latestId, self.client.rulesKey = id, fields[16]
    if view.id ~= id then
        Games.Comms:Cancel("answer")
        self.client.answerRevision, self.client.ackVersion = 0, -1
        self.client.ackChoice, self.client.ackRevision = nil, nil
        self.client.lockSyncId = nil
        self.view = {
            role = "participant",
            state = "posting",
            gameTypeId = Quiz.id,
    ...
    Send(self.client.name, Fields("D", self.client.session, id, self.client.request))
...
function Session:ReceiveResult(fields, now)
    ...
    local score = TotalScore(fields[7])
    ...
    local rules = Quiz.Rules.Decode(fields[20])
    local duration = rules and Integer(fields[15], rules.answerSeconds, rules.answerSeconds)
    local elapsed = fields[17] ~= "" and Elapsed(fields[17], duration) or nil
    local fastestName = fields[18] ~= "" and Games.Identity:NormalizeName(fields[18]) or nil
    ...
    if
        ...
        or selected == correct and streak == 0
        or selected ~= correct and (streak ~= 0 or streakBonus ~= 0)
        or streakBonus ~= Quiz.Scoring.StreakBonus(rules, streak)
        or selected and points ~= Quiz.Scoring.Calculate(selected == correct, elapsed, duration, rules, streak)
    ...
    if
        fastestName
        and fastestName:lower() == Games.Identity.name:lower()
    ...
    Standings:MarkDirty(self, id)
    client.rulesKey = fields[20]
    local tag = "result-ack:" .. fields[3]
    Games.Comms:Cancel(tag)
    Send(client.name, Fields("F", client.session, id, client.request), tag)
    if
...
function Session:ReceiveParticipant(fields, now)
    ...
    local code = fields[1]
    if code == "W" then
        local score = TotalScore(fields[4])
        if
        ...
        Send(client.name, Fields("S", client.session, client.request))
        return
    end
    ...
    elseif code == "X" then
        ...
        Standings:Reset(self)
    elseif code == "V" then
        if not Standings:ReceiveView(self, fields, now) then
            return
        end
    elseif code == "H" then
        local id = Integer(fields[4], 0, MAX_INTEGER)
        local score = TotalScore(fields[5])
        if
    ...
    elseif code == "P" then
        ...
        if code == "O" then
            ...
            if not expiry or expiry > GetServerTime() + view.duration + EXPIRY_GRACE then
                return
            end
            if view.correctIndex then
                return
            end
            if not view.deadline or view.serverDeadline ~= expiry then
                ...
                view.serverDeadline = expiry
            end
            view.state, view.notice = now < view.deadline and "open" or "results", nil
            view.locked = view.state ~= "open"
                or client.lockSyncId == id
                or view.selected ~= nil and not view.rules.allowAnswerChanges
        elseif code == "K" then
            ...
            if view.rules.allowAnswerChanges and revision < client.answerRevision and view.pending then
                return
            end
            Games.Comms:Cancel("answer")
            view.selected, view.locked, view.pending, view.notice =
                choice, not view.rules.allowAnswerChanges, false, nil
        elseif code == "E" then
            local revision = Integer(fields[5], 1, MAX_INTEGER)
            if
            ...
            local answerLocked = fields[4] == "answer_locked"
            client.lockSyncId = answerLocked and not view.confirmedSelected and id or nil
            view.locked = answerLocked or view.selected ~= nil and not view.rules.allowAnswerChanges
            if client.lockSyncId then
                client.nextSyncAt = now
            end
        else
            return
        end
...
function Session:Tick(now)
    ...
    if self.hostSession then
        ...
        if now >= self.nextHeartbeat then
            ...
            local game = Quiz.Controller.game
            for _, peer in pairs(self.peers) do
                if not Games.Comms:IsBusy(peer.name) then
                    Send(
                        peer.name,
                        Fields(
                            "H",
                            self.hostSession,
                            game.state == "finished" and "results" or game.state,
                            game.lastQuestionId,
                            ScoreText(Quiz.Controller:GetScore(peer.playerKey))
                        ),
                        PeerTag(peer.name)
    ...
    elseif self.client then
        ...
        if view.pending and view.state == "open" and now >= client.retryAt then
            if client.answerAttempts < MAX_ANSWER_ATTEMPTS then
                ...
                Send(client.name, AnswerFields(client, view), "answer")
            else
        ...
        if
            (
                view.state == "waiting"
                or view.state == "posting"
                or view.state == "results" and not view.correctIndex
                or view.state == "open" and client.lockSyncId == view.id
        ...
        if now >= client.nextHeartbeat then
            ...
            if view.state == "disconnected" then
                ...
                Send(client.name, Fields("T", client.session, client.request))
            end
        end
