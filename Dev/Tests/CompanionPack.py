...
EXAMPLE = ROOT / "Dev" / "Examples" / "Orbit-Quiz-Pack-Example"
...
def registration_api(lua, quiz):
    return lua.eval("""function(core)
        local registrations = {}
        local api = {}
        function api:RegisterQuestionPack(pack)
            registrations[#registrations + 1] = pack
            return core:RegisterQuestionPack(pack)
        end
        setmetatable(api, { __index = function(_, key)
            error("Companion accessed a private Orbit-Quiz member: " .. tostring(key))
        end })
...
def run_suite():
    ...
    check(required == ["Orbit-Quiz"], "The companion's only required addon is Orbit-Quiz")
    ...
    lua, quiz = runtime(development=False)
    test, globals_ = lua.globals().Test, lua.globals()
    same = lua.eval("rawequal")
    bundled = quiz.GetQuestionPacks(quiz)
    bundled_count = len(quiz.GetQuestions(quiz, "all"))
    check(len(bundled) == 1 and bundled[1].id == "warcraft-lore",
          "Packaged Orbit-Quiz must contain only its bundled quiz before installing the companion")
    check(quiz.Development is None and globals_.SlashCmdList.ORBITQUIZDEV is None,
    ...
    globals_.OrbitQuiz = public_api
    try:
        for path in paths:
            load(lua, path, companion_namespace, EXAMPLE.name)
        check(same(globals_.OrbitQuiz, public_api), "Pack authors must not replace the framework's public global")
    finally:
        globals_.OrbitQuiz = quiz
    ...
    packs = {pack.id: pack for pack in quiz.GetQuestionPacks(quiz).values()}
    ...
    rules, rules_key = quiz.GetPackRules(quiz, PACK_ID)
    ...
    test.Advance(30)
    check(quiz.Main.game is None and not quiz.Main.IsRunning(quiz.Main), "Installing a pack must not start a game")
    check(quiz.Session.hostSession is None and quiz.Session.client is None, "Installing a pack must not create a session")
    check(quiz.UI.frame is None and quiz.Widget.frame is None, "Installing a pack must not open quiz windows")
    check(quiz.Store.GetSettings(quiz.Store).packId == saved_selection, "Installing a pack must not change host selection")
    check(globals_.Orbit is None and quiz.Development is None, "The companion must not create hidden addon dependencies")
    ...
    check(quiz.Main.Start(quiz.Main, settings) is True, "The host can explicitly start the independently installed quiz")
    check(quiz.Main.game.state == "open" and quiz.Main.game.round.packId == PACK_ID,
          "The standalone pack must produce a playable question without development code")
    check(quiz.Main.game.rulesKey == rules_key, "Hosting the companion must use the author's rule identity")
    ...
    check(metadata.get("dependencies") == "Orbit-Quiz", "The guide's addon must require the installed framework")
    ...
    check({pack.id for pack in quiz.GetQuestionPacks(quiz).values()} == {"warcraft-lore", "myguild_trivia"},
    ...
    rules, _ = quiz.GetPackRules(quiz, "myguild_trivia")
    ...
    check(quiz.Main.game is None and quiz.Session.hostSession is None and quiz.Session.client is None,
          "The guide must not automatically host or join a game")
    check(quiz.Development is None and globals_.Orbit is None, "The guide must not need development code or Orbit")
    return assertions
