...
function Widget:Create()
    ...
    self.scoreText:Hide()
    self.scoreAnimation = self.scoreText:CreateAnimationGroup()
    self.scoreAnimation:SetLooping("NONE")
    self.scoreRise = self.scoreAnimation:CreateAnimation("Translation")
    self.scoreRise:SetOrder(1)
    self.scoreRise:SetDuration(SCORE_ANIMATION_SECONDS)
    self.scoreRise:SetSmoothing("OUT")
    self.scoreFade = self.scoreAnimation:CreateAnimation("Alpha")
    self.scoreFade:SetOrder(1)
    self.scoreFade:SetFromAlpha(1)
    self.scoreFade:SetToAlpha(0)
    self.scoreFade:SetStartDelay(SCORE_FADE_DELAY)
    self.scoreFade:SetDuration(SCORE_ANIMATION_SECONDS - SCORE_FADE_DELAY)
    self.scoreFade:SetSmoothing("IN")
    local function HideScoreText()
        self.scoreText:Hide()
    end
    self.scoreAnimation:SetScript("OnFinished", HideScoreText)
    self.scoreAnimation:SetScript("OnStop", HideScoreText)
    ...
    self.winnerAnimation = self.winnerText:CreateAnimationGroup()
    self.winnerAnimation:SetLooping("NONE")
    self.winnerRise = self.winnerAnimation:CreateAnimation("Translation")
    self.winnerRise:SetOrder(1)
    self.winnerRise:SetDuration(SCORE_ANIMATION_SECONDS)
    self.winnerRise:SetSmoothing("OUT")
    self.winnerFade = self.winnerAnimation:CreateAnimation("Alpha")
    self.winnerFade:SetOrder(1)
    self.winnerFade:SetFromAlpha(1)
    self.winnerFade:SetToAlpha(0)
    self.winnerFade:SetStartDelay(SCORE_FADE_DELAY)
    self.winnerFade:SetDuration(SCORE_ANIMATION_SECONDS - SCORE_FADE_DELAY)
    self.winnerFade:SetSmoothing("IN")
    local function HideWinnerText()
        self.winnerText:Hide()
    end
    self.winnerAnimation:SetScript("OnFinished", HideWinnerText)
    self.winnerAnimation:SetScript("OnStop", HideWinnerText)
    self.timer = CreateFrame("StatusBar", nil, self.questionContent)
    self.timer:EnableMouse(false)
    self.timer:SetMinMaxValues(0, Quiz.ANSWER_SECONDS)
...
function Widget:RenderQuestion(view, changed)
    ...
    PixelUtil.SetSize(self.scoreText, math.min(SCORE_WIDTH, width / 3), MIN_TEXT_HEIGHT)
...
function Widget:RenderTimer(remaining)
    local fraction = Clamp(remaining / Quiz.ANSWER_SECONDS, 0, 1)
...
function Widget:UpdateTimer()
    local remaining = Clamp(self.timerDeadline - GetTime(), 0, Quiz.ANSWER_SECONDS)
...
function Widget:Refresh()
    ...
    local changed = self.questionId ~= view.id
        or self.questionCycle ~= view.cycle
        or self.questionHost ~= view.hostName
        or self.previousState ~= view.state
    if changed then
    ...
    self:RenderQuestion(view, changed)
    self:RenderScore(view)
    self:RenderWinner(view)
    self.timerDeadline = view.state == "open" and view.deadline or nil
    local remaining = self.timerDeadline and Clamp(self.timerDeadline - GetTime(), 0, Quiz.ANSWER_SECONDS) or 0
    local canAnswer = remaining > 0
    self:RenderTimer(view.state == "preview" and Quiz.ANSWER_SECONDS or remaining)
    self.timer:SetScript("OnUpdate", canAnswer and not self.timerFailed and AnimateTimer or nil)
