...
function UI:CreateHostPage(page)
    ...
    self.durationInfo = Label(page, L.W_FIXED_DURATION, 0, -80, CONTENT_WIDTH, FIELD_HEIGHT)
    Controls:SetMuted(self.durationInfo)
    self.hostHelp = Label(page, L.W_HOST_HELP, 0, HOST_HELP_Y, CONTENT_WIDTH, HOST_HELP_HEIGHT)
...
function UI:Create()
    ...
    self.headerDivider = Quiz.DialogChrome:Divider(frame, CONTENT_WIDTH)
    ...
    self.footerDivider = Quiz.DialogChrome:Divider(frame, CONTENT_WIDTH)
...
function UI:GetBoardRows(view)
    ...
    if self.boardScope == "personal" then
        for _, pack in ipairs(Quiz.PersonalScores:GetPacks()) do
            rows[#rows + 1] =
                L.W_PACK_SCORE_ROW_F:format(pack.title, pack.score, pack.correct, pack.incorrect, pack.answers)
        end
...
function UI:Refresh()
    ...
    elseif self.tab == "host" then
        ...
        Controls:SetButtonState(self.start, not running and not Quiz.Main:IsRestricted())
        Controls:SetButtonState(self.pause, running)
