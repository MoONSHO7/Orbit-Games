return function(Quiz)
    ...
    local function Advert(session, pack, league, state, players)
        return table.concat(
            { "6", "A", session or "101.1", pack or "First pack", league or "League", state or "open", players or "1" },
            "|"
        )
    end
    ...
    Tick(103)
    Same(sent[1].text, "6|Q", "a lookup follows automatic join")
    ...
    Check(Receive("6|X|101.1"), "stale withdrawal is harmless")
    Same(Discovery:GetGames()[1].session, "101.2", "stale withdrawal does not close the replacement")
    Check(Receive("6|X|101.2"), "matching stop withdraws a game")
    ...
    for _, packet in ipairs({
        ...
        "6|A|101.1|Pack|League|open|0",
        "6|A|101.1|Pack|League|open|18",
        "6|A|101.1|Pack|League|open|nan",
        "6|A|101.1|Pack|League|open|1.5",
        "6|A|101.1|Pack|League|finished|1",
        "6|A|101.1|Pack|League|open|1|extra",
        "6|A||Pack|League|open|1",
        "6|A|101.1||League|open|1",
        "6|A|101.1|Pack||open|1",
        "6|A|101.1|{rt1}|League|open|1",
        "6|A|101.1|Pack\nInjected|League|open|1",
        "6|A|101.1|Pack\000Injected|League|open|1",
        "6|A|101.1|Pack\127Injected|League|open|1",
    ...
    Fresh()
    Host()
    Check(Receive("6|Q", "Alice-TestRealm", "WHISPER"), "explicit native probe is handled")
    Same(Discovery.queueCount, 1, "one private reply is scheduled")
    for _ = 1, 20 do
        Check(Receive("6|Q", "Alice-TestRealm", "WHISPER"), "repeated query is handled")
    end
    ...
    Check(sent[1].text:match("^6|A|100.1|"), "host reply advertises only the current game")
    Same(Quiz.Session.client, nil, "host-side discovery also cannot join anything")
    Fresh()
    Same(Receive("6|Q", "Alice-TestRealm", "WHISPER"), true, "idle client quietly handles a query")
    ...
    Host()
    for index = 1, 100 do
        Check(Receive("6|Q", Player(index), "WHISPER"), "native lookup flood remains harmless")
    end
    ...
    for _, entry in pairs(Discovery.queue) do
        Check(not entry.message:match("^6|A|"), "withdrawal removes unsent advertisements")
    end
    Tick(101)
    Check(sent[#sent].text:match("^6|X|100.1$"), "stop publishes a bounded withdrawal")
    Flush(5)
    for index = 2, #sent do
        Check(not sent[index].text:match("^6|A|100.1|"), "same stopped session is never readvertised")
    end
    Host("100.2")
    Flush(6)
    local foundReplacement = false
    for _, packet in ipairs(sent) do
        foundReplacement = foundReplacement or packet.text:match("^6|A|100.2|") ~= nil
    end
