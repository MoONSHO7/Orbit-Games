...
return function(Quiz)
    ...
    local function ScoreGeometry()
        ...
        Near(
            label:GetWidth(),
            PixelUtil.GetNearestPixelSize(math.min(SCORE_WIDTH, Widget.renderedWidth / 3), label:GetEffectiveScale()),
    ...
    local function TimerGradient(remaining)
        ...
        local fraction = math.max(0, math.min(1, remaining / 15))
    ...
    local function TimerGeometry()
        ...
        Same(maximum, 15, "countdown maximum is the fixed fifteen seconds")
    ...
    local function Advertise(name, session, players)
        Check(
            Quiz.Discovery:Receive(
                PREFIX,
                "6|A|" .. session .. "|Test pack|Default|open|" .. players,
                "GUILD",
                name,
                "",
                0,
    ...
    local dividers = 0
    for divider in pairs(Quiz.DialogChrome.dividers) do
    ...
    Check(UI.hostHelp:IsShown(), "automatic-play guidance is always visible in Host")
    Check(UI.hostHelp:GetText():find("automatically", 1, true), "Host explains automatic question progression")
    Check(UI.hostHelp:GetText():find("Everyone needs Orbit-Quiz", 1, true), "Host explains addon-only participation")
    Check(UI.hostHelp:GetTop() < UI.durationInfo:GetBottom(), "guidance follows the fixed-duration rule")
    ...
    for count = 6, 4, -1 do
        ...
        for index, button in ipairs(Widget.choices) do
            ...
            if index <= count then
                Same(button.Text:GetText(), fields[15 + index], "answer labels display only original pack text")
