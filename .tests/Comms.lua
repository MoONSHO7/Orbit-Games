return function(Quiz)
    ...
    Fresh()
    Same(Comms:NormalizeName("  Alice  "), "Alice-TestRealm", "local realm supplied")
    Same(Comms:NormalizeName("Alice-ForeignRealm"), "Alice-ForeignRealm", "foreign realm preserved")
    Same(Comms:NormalizeName("Élise-Éitrigg"), "Élise-Éitrigg", "UTF-8 identity retained")
    Same(Comms:NormalizeName("Alice-Azjol-Nerub"), "Alice-Azjol-Nerub", "realm hyphens retained")
    Same(Comms:NormalizeName("Alice-Kel'Thuzad"), "Alice-Kel'Thuzad", "realm apostrophe retained")
    for _, name in ipairs({
        ...
        Same(Comms:NormalizeName(name), nil, "invalid name rejected")
    end
    Same(Comms:NormalizeName(secret), nil, "secret name rejected before use")
    Same(Comms:NormalizeName(12), nil, "numeric name rejected")
    realm = secret
    Same(Comms:NormalizeName("Alice"), nil, "secret local realm rejected")
    Same(
        Comms:NormalizeName("Alice-OtherRealm"),
        "Alice-OtherRealm",
        "explicit foreign realm needs no local realm query"
    )
    realm = "Bad Realm"
    Same(Comms:NormalizeName("Alice"), nil, "invalid native realm rejected")
    realm = "TestRealm"
    Same(#Comms:NormalizeName(string.rep("A", 118) .. "-R"), 120, "maximum full name accepted")
    ...
    Same(Comms:Receive("ORBITQUIZ2", valid, "WHISPER", "Alice"), false, "four-choice clients cannot enter protocol six")
    Same(
        Comms:Receive("ORBITQUIZ3", valid, "WHISPER", "Alice"),
        false,
        "old zero-penalty clients cannot enter protocol six"
    )
    Same(
        Comms:Receive("ORBITQUIZ4", valid, "WHISPER", "Alice"),
        false,
        "old league-score clients cannot enter protocol six"
    )
    Same(Comms:Receive("ORBITQUIZ5", valid, "WHISPER", "Alice"), false, "pre-winner clients cannot enter protocol six")
    ...
    Same(#largestQuestion, 17, "largest quiz payload stays below the thirty-two-field codec bound")
    for index = 1, 16 do
        Check(Comms:Send(Player(index), largestQuestion), "maximum six-choice payload queues for every participant")
    end
    Same(Comms.queueCount, 96, "maximum bounded question uses six fragments per participant")
    Drain()
    Same(#sent, 96, "all maximum-size questions drain without increasing the packet rate")
    Same(#failures, 0, "queue lifetime covers a complete sixteen-player question batch")
    Check(now < 112, "maximum question batch still fits the twelve-second readiness allowance")
    for _, packet in ipairs(sent) do
    ...
    for _, message in ipairs(received) do
        Same(#message.fields, 17, "reassembled maximum question retains every field")
        Same(message.fields[17], largestQuestion[17], "the sixth maximum-length choice survives transport")
    end
