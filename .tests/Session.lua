...
return function(Quiz)
    ...
    local function Question(id, choices, difficulty, era)
    ...
    Same(#question, 19, "four-choice question contains pack provenance before the shuffled choices")
    ...
    for index = 1, 4 do
        Same(question[15 + index], round.choices[index], "question packet carries only shuffled choice text")
    end
    ...
    Same(#result, 19, "result carries a per-pack receipt and finalized winner metadata")
    ...
    for _, oldDuration in ipairs({ 10, 20, 60 }) do
        ...
        Comms:Clear()
        Unchanged(Changed(Question(100), 7, tostring(oldDuration)), "protocol six rejects an obsolete answer duration")
    ...
    Check(not Session:CanOpen(Test.now + 11.999), "unready peer is allowed the bounded preparation window")
    Check(Session:CanOpen(Test.now + 12), "preparation does not wait indefinitely for a peer")
    Incoming(PeerFields("D", sessionId, round.id), PEER)
    ...
    for _, fields in ipairs({
        ...
        Changed(Question(71), 17, string.rep("x", 101)),
        Changed(Question(71), 18, "choice\n"),
        Changed(Question(71), 19, "choice|markup"),
        Changed(Question(71), 19, " red "),
        Changed(Question(71), 16, "choice|markup"),
    ...
    Unchanged(oldResult, "protocol six rejects the prior seventeen-field receipt shape")
