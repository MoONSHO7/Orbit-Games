...
COMIC_ARCHIVE = ROOT / "Packs" / "Archive" / "Comics"
...
def active_lore_files():
    return sorted(path for path in (ROOT / "Packs").glob("Lore*.lua") if path.name != "LoreData.lua")
...
def read_lore_files(paths):
    ...
    origins = {}
    for path in [ROOT / "Packs" / "LoreData.lua", *paths]:
...
def run_suite(namespace):
    ...
    authored_files = {"Packs/LoreData.lua", *("Packs/" + path.name for path in active_lore_files())}
    for development in (True, False):
        ...
        check(not any(path.startswith("Packs/Archive/") or Path(path).name in ARCHIVED_COMIC_FILES for path in scripts),
    ...
    check(not (ROOT / "Packs" / "ResearchComics.md").exists(), "Comic research must not remain in the active pack folder")
    for name in ARCHIVED_COMIC_FILES:
        check(not (ROOT / "Packs" / name).exists(), "Comic files cannot remain in the active draft search: " + name)
