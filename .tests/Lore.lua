...
return function(Quiz)
    ...
    Check(Quiz.PersonalScores:RecordResult(oldReceipt), "a prior pack-one answer is saved under the original pack ID")
    local oldSave = Quiz.Store.db
    ...
    Check(
        oldSave.personalScores.packs[PACK_ID].scoringVersions[Quiz.Scoring.VERSION].score == 2,
        "loading and continuing leave the original save untouched"
    )
    local stored, reason = Quiz.PersonalScores:RecordResult(oldReceipt)
    Check(stored and reason == "duplicate", "an old result replay remains a duplicate after comic removal")
