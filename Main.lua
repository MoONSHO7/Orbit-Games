...
function Main:IsRunning()
    return self.game ~= nil and self.game.state ~= "finished" and self.game.state ~= "stopped"
end
...
function Main:SyncRestriction()
    ...
    if restricted and self:IsRunning() and self.game.state ~= "paused" then
...
function Main:PlayerKey(name, trustedGUID)
    local fullName = Quiz.Comms:NormalizeName(name)
...
function Main:Start(settings)
    ...
    local game, gameError = Quiz.Game.New(configured, questions, math.random)
    if not game then
    ...
    self.notice, self.nextAutoAt, self.autoPaused = nil, nil, false
...
function Main:AcceptAnswer(name, trustedGUID, id, choice, actionId)
    ...
    local accepted, reason = self.game:Submit(key, Quiz.Comms:NormalizeName(name), id, choice, now, actionId)
    if accepted then
        Quiz.UI:Refresh()
        Quiz.Widget:Refresh()
    end
...
function Main:GetHostView()
    ...
    if round then
        ...
        view.scoringVersion = Quiz.Scoring.VERSION
        view.difficulty, view.era = round.difficulty, round.era
        view.duration, view.deadline = Quiz.ANSWER_SECONDS, round.deadline
        local answer = round.answers[Quiz.Identity.guid]
        view.selected, view.locked = answer and answer.choiceIndex or nil, game.state ~= "open"
...
function Main:CloseQuestion(now)
    ...
    self.nextAutoAt = now + INTERMISSION
end
...
function Main:Stop()
    ...
    self.nextAutoAt, self.autoPaused = nil, false
...
function Main:Command(message)
    ...
    elseif command == "scores" then
        local packs = Quiz.PersonalScores:GetPacks()
        Quiz:Print(L.W_PERSONAL_BOARD)
        if #packs == 0 then
            Quiz:Print(L.W_NO_PERSONAL_SCORES)
        end
        for _, pack in ipairs(packs) do
            Quiz:Print(L.W_PACK_SCORE_ROW_F:format(pack.title, pack.score, pack.correct, pack.incorrect, pack.answers))
        end
