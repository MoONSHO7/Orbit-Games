...
local function HostedGame(self)
    ...
    if not session or session == self.withdrawnSession or not game or not HOST_STATES[game.state] then
        return nil
    end
    ...
    return table.concat({ WIRE_VERSION, "A", session, packName, game.settings.league, game.state, players }, "|"),
        session
end
...
function Discovery:Start()
    ...
    if not issecretvalue(name) and not issecretvalue(realm) and type(name) == "string" then
        self.localName =
            Quiz.Comms:NormalizeName(type(realm) == "string" and realm ~= "" and name .. "-" .. realm or name)
    end
    self.started = true
...
function Discovery:Probe(hostName)
    ...
    local name = Quiz.Comms:NormalizeName(hostName)
...
function Discovery:Receive(prefix, text, distribution, sender, target, _, localID, channelName)
    ...
    elseif distribution == "WHISPER" then
        local recipient = Quiz.Comms:NormalizeName(target)
