...
From the addon folder, run `python Dev/Tests/Lore.py` for the shipped pack, or `python Dev/Tests/Lore.py --draft --review` while editing era files. Run `python Dev/Tests/run.py` for registration, shuffle, scoring, protocol and widget regressions. These checks do not replace reading the cited evidence or testing in WoW.
