...
The addon includes **1,183 game-based Warcraft Lore questions**, covering the RTS games and every released WoW expansion through Midnight. Zones, areas, regional history, campaigns, characters, raids and quests are included; comic, graphic-novel and manga questions are not. It mixes easy, medium, hard and very hard questions, with four to six choices each. It contains full story spoilers; see [the coverage and source guide](Packs/SOURCES.md).

Add your own packs using the copyable companion addon in [PACKS.md](PACKS.md): a pack is a Lua file listed in its own addon's TOC, installed beside Orbit-Quiz. There is no editor or automatic discovery of loose YAML, JSON, Markdown, or text files. Only install Lua addons from trusted authors. Existing four-choice packs remain valid.

## Host a quiz

1. Open `/oq` and select **Host**.
2. Choose a question pack. Every hosted question has exactly **15 seconds** to answer; there is no duration option. The default game name works without typing. Game names label listings; personal scores always belong to the question's pack.
...
The quiz uses every question in the selected pack, shuffling both questions and choices. Each question runs for fifteen seconds, then reveals the answer for three seconds before preparing the next question automatically. Connected players still synchronize before the next fifteen-second clock opens. At the end of the pack it starts another shuffled cycle; the last question is not immediately repeated when there is more than one question. One-question packs necessarily repeat.

Warcraft Lore content version 2 removes the previous 247 comic-based questions without resetting earned scores. After updating an already-running host, `/reload` and start a new game to use the trimmed deck. Source checkouts retain the removed files under `Packs/Archive/Comics` for recovery; that archive is never loaded or included in packaged releases.
...
4. You can change your selection before the deadline. The latest change accepted by the host replaces the previous answer and uses its new receipt time for scoring. Clicking the already selected answer does nothing.

The widget shows the pack name in small grey text above the question, then a two-pixel countdown bar and a vertical list of answer text without letter prefixes. The question uses a larger font (+4); long questions and answers wrap. The bar sits two pixels below the question and changes its horizontal gradient from gold through orange to red as the fifteen seconds run out. Hover lights up an answer; pressing nudges and dims its text, then releasing restores it. Gold marks your selection. The hit area and surrounding layout stay still.
...
Sessions support one host and up to 16 remote widget players. Install this build on each client: protocol 6 intentionally isolates earlier builds whose result messages lack fastest-player metadata. Only the host needs extra packs. Discovery uses a silently joined `OrbitQuizLobby` custom channel plus available guild/group routes. This lobby is not attached to a visible chat tab. Questions and answers use native addon whispers, not visible whispers or chat lines.
...
The bare widget follows Orbit's Error Messages positioning: horizontal screen thirds align the pack name and question; the upper/lower half determines which way content grows. Answers stay left-aligned in one column. Long content scrolls within a bounded area, with space reserved beside the prompt for the score and below the answers for the winner. Neither animation moves the question or answer rows. Position is saved separately from host setup and reflows when display size or UI scale changes; the timer stays exactly two physical pixels thick.
...
Correct answers earn `1 + 0.1 × whole seconds remaining`. Wrong answers lose between `1.0` and `0.5` points: the faster the final selection, the larger the loss. Unanswered questions score `0`. With the fixed fifteen-second window:
...
The wrong-answer loss is `round-to-tenth(0.5 + 0.5 × (2^(-t/6) − 2^(-2.5)) / (1 − 2^(-2.5)))`, where `t` is seconds since answers opened. This is an exponential curve normalized to the endpoints, with a six-second half-life and tenth-point steps. Uniform random guessing has a negative expected score at every answer time for four, five or six choices; it is not a guarantee that an individual lucky guess loses. Totals can go below zero, and no-answer is always zero.
...
The **Scores** tab opens **My pack scores**: your points, correct/wrong counts and answered questions for each quiz pack. `/oq scores` prints the same progress. These totals follow you between hosts and are shared by your characters in the same local WoW account. All packs games credit each question's original pack. Changing a pack's title or publishing a new content version under the same ID retains its progress; another pack ID starts separately.
...
**Archived league totals** and **Archived 100-point scores** retain the earlier host-owned standings, grouped by their original league names; `/oq archive` and `/oq legacy` print them. Schema 6 preserves prior totals/history, appearance settings, question counters and historical scoring rules without rescoring or assigning old totals to a guessed pack. New personal progress starts with the first confirmed result after upgrading. Retired chat settings/passwords are discarded, and old Warcraft Basics selections become Warcraft Lore.
...
When the host's addon communication is restricted or its chat server disconnects, the quiz automatically pauses and voids the unfinished question. Once available again it resumes with a fresh question. A manually paused quiz stays paused. A restricted participant disables input, then re-registers and waits for current host state. Older answers and acknowledgements cannot overwrite the recovered selection.
...
- Install this build on both clients, but a test companion pack only on the host. Join through Games. Confirm the small grey pack label, larger question, one-column answers without letter prefixes and two-pixel countdown. Verify fifteen seconds to answer, three seconds of results, then automatic progress without visible chat. Correct/wrong answers animate each player's own signed score beside the question; skipping does not. Below the answers, both clients should see the same fastest-correct name/time; a faster wrong guess must not win. Check solo, nobody correct, and one player skipping: only the first two suppress the winner.
...
- Complete more than one full cycle using a small pack. Confirm three-second results, shuffled cycles, no immediate boundary repeat for a multi-question pack, and Stop ending progression.
...
- Finish correct and incorrect answers, reload each client and join a second host using the same pack. My pack scores must retain and accumulate signed totals; This game must start fresh. Play another pack and an All packs game: each question credits only its source pack. Check a second local character shares the personal totals. Previous league and 100-point archive views must remain unchanged after new gameplay.
