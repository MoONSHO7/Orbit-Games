...
`Game.lua` snapshots the selected rules and owns deck progression, answer revisions, session standings, streaks, voids and finite completion. Its caller supplies time/randomness; no UI or transport drives the model internally. `Scoring.lua` applies current version-3 scoring and preserves the explicit version-2 calculator and deterministic standings ordering.
...
- Streaks advance only at closure. Wrong/unanswered closed rounds reset them, while voids do not; each new game starts at zero standings and streaks.
