...
function Session:JoinHost(name, advertisedSession)
    ...
    local host = Quiz.Identity:NormalizeName(name)
    if not host then
...
function Session:OpenRound()
    self.serverDeadline = GetServerTime() + Quiz.Main.game.rules.answerSeconds
    for _, peer in pairs(self.peers) do
        if peer.readyId == Quiz.Main.game.round.id then
            self:SendOpen(peer)
        end
...
function Session:ReceiveResult(fields, now)
    ...
    local fastestName = fields[18] ~= "" and Quiz.Identity:NormalizeName(fields[18]) or nil
