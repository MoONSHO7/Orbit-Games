...
local function IsPinned(session, id, entry)
    if entry.name:lower() == Quiz.Identity.name:lower() or session.peers[entry.name:lower()] then
        return true
    end
    local result = Quiz.Main.game and Quiz.Main.game.lastResult
...
function StreakSync:Register(session, name)
    name = Quiz.Identity:NormalizeName(name)
    if not name then
        return nil
    end
    ...
    if speakers.count >= MAX_SPEAKERS then
        ...
        self:Register(session, Quiz.Identity.name)
        for _, peer in pairs(session.peers) do
...
function StreakSync:RefreshView(client, view)
    ...
    for _, reference in ipairs(view.streakReferences) do
        ...
        if not entry then
            ...
            if names[key] or key == Quiz.Identity.name:lower() and reference.streak ~= view.streak then
    ...
    if not incomplete and view.streak >= MIN_STREAK and not names[Quiz.Identity.name:lower()] then
        view.suppressStreakToasts = true
        return
    end
...
function StreakSync:ReceiveName(client, view, fields)
    ...
    for index, idText in ipairs(ids) do
        local id, name = Integer(idText, 1), Quiz.Identity:NormalizeName(names[index])
        if
            not id
            or not name
            or name ~= names[index]
    ...
    Quiz.Comms:Send(client.name, { "B", client.session, client.request, fields[4] })
...
function StreakSync:TickHost(session, now)
    ...
    if chosenPeer then
        ...
        if
            Quiz.Comms:Send(chosenPeer.name, {
                "N",
                session.hostSession,
                chosenPeer.request,
...
function StreakSync:TickClient(client, view, now)
    ...
    for _, reference in ipairs(view.streakReferences or {}) do
        if not client.streakSpeakers or not client.streakSpeakers.entries[reference.id] then
            if Quiz.Comms:Send(client.name, { "U", client.session, client.request, IdText(reference.id) }) then
